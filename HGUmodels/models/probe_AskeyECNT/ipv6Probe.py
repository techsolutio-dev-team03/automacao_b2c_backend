...
class HGU_AskeyECNT_ipv6Probe(HGU_AskeyECNT):
    ...
    def iPerf2PCsClientServer_230(self, flask_username,  ipv_x, dhcpv6):
        ...
        for ip_remoto in ip_remoto_list[2:]:
            try:
    ...
    def iPerf2PCsServerClient_231(self, flask_username,  ipv_x, dhcpv6):
        ...
        iperf_server = subprocess.Popen('iperf3 -s', shell=True, preexec_fn=os.setsid)
        ...
        ssh.close()
        self._driver.quit()
        os.killpg(os.getpgid(iperf_server.pid), signal.SIGTERM)
        return self._dict_result 
    ...
    #             print("\n", ip_wifi)
        ...
        try:
            ...
            # time.sleep(1000)
            resultado = os.popen(f'iperf3 -c {ip6_remoto}')
            time.sleep(12)
            # print(resultado.error)
            # output = teste.recv(65000).decode('utf-8')
    #             print("\n", output)
